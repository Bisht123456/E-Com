...
const dir = `${baseDir}/documents/productImage`;

const writeFiles = async ({ images, thumbnail }) => {
  ...
  if (images && images.length > 0) {
    await Promise.all(
      images.map((file) => {
        fs.writeFile(
          path.resolve(dir, `${file.originalname}`),
          file.buffer,
          () => console.log("image downloaded3")
        );
      })
    );
  }
...
const postproduct = expressAsyncHandler(async (req, res) => {
  ...
  try {
    ...
    await writeFiles({ images, thumbnail });
    ...
    if (thumbnail && thumbnail.length > 0) {
      const fileExist = thumbnail.find((image) => image.originalname);
      if (fileExist) {
        userObj["thumbnail"] = fileExist.originalname;
      }
    }

    const productadd = await Userproducts.create(userObj);

    userData.image.map(async (item) => {
      if (images && images.length > 0) {
        const fileExist = images.find(
          (file) =>
            file.originalname.split("_")[0].replace(/\.[^/.]+$/, "") ==
            item.uniqId
        );
        if (fileExist) {
          item.file = fileExist.originalname;
        }
      }
      await ProductImage.create({
        product_id: productadd._id,
        images: item.file,
      });
    });

    res.status(200).json({ message: "Success: Product uploaded." });
  } catch (error) {
    console.error(error);
      ...
      const productsQuery = [
        ...
        },
        {
          $lookup: {
            ...
            localField: "type_subcategory_id",
            foreignField: "_id",
            as: "typeSubcategory",
          },
        },
...
const updateproduct = expressAsyncHandler(async (req, res) => {
  try {
    ...
    const dataproduct = await Userproducts.findOne({ _id: product });

    const updateFields = {};

    if (thumbnail && thumbnail.length > 0) {
      const fileExist = thumbnail.find((file) => file.originalname);
      let dbFile = dataproduct.thumbnail;
      if (fileExist) {
        if (dbFile) {
          if (fs.existsSync(path.resolve(dir, `${dbFile}`)))
            fs.unlinkSync(path.resolve(dir, `${dbFile}`));
        }
        updateFields.thumbnail = fileExist.originalname;
      }
    }
    ...
    userData.image.map(async (item) => {
      ...
      if (images && images.length > 0) {
        ...
        let dbFile = productImage.images;
        if (fileExist) {
          if (dbFile) {
            if (fs.existsSync(path.resolve(dir, `${dbFile}`)))
              fs.unlinkSync(path.resolve(dir, `${dbFile}`));
          }
          item.images = fileExist.originalname;
        }
      }
    ...
    //   console.log(item.images, "productImage", item._id);
    //   // if (images && images.length > 0) {
    //   //   const fileExist = images.find(
    //   //     (file) =>
    //   //       file.originalname.split("_")[0].replace(/\.[^/.]+$/, "") ==
    ...
    //   //       if (fs.existsSync(path.resolve(dir, `${dbFile.file}`)))
    //   //         fs.unlinkSync(path.resolve(dir, `${dbFile.file}`));
    //   //     }
    //   //     updateFields.thumbnail = fileExist.originalname;
    //   //   }
    //   // }
